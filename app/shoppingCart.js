//----------------------------------------------------------------
// shopping cart
// gestion de pannier avec le local storage du navigateur
//
function shoppingCart(cartName) {
    ...
    this.loadItems();
    // eitems to local storage when unloading
    var self = this;
}

// chargement des items dans le locale storage
        ...
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (item.sku != null && item.name != null && item.image  != null && item.price != null && item.quantity != null) {
                    item = new cartItem(item.sku, item.name,item.image,item.description,item.poids, item.price, item.quantity);
...
// enregistrement des  items dans local storage
...
shoppingCart.prototype.addItem = function (sku, name,image,description,poids, price, quantity) {
    quantity = this.toNumber(quantity);
    if (quantity != 0) {

        // mise à jour  de la quantité pour l'item existant
        var found = false;
        ...
        // nouveau item,ajout maintenent
        if (!found) {
            var item = new cartItem(sku, name,image,description,poids, price, quantity);
            this.items.push(item);
        }
...
//Envoi le prix total pour tous les items courant dans le panier
...
// items in the cart
//
function cartItem(sku, name,image,description,poids, price, quantity) {
    this.sku = sku;
    this.image = image;
    this.name = name;
